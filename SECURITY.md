...
Trusted repository verification enters through `scripts/run-make.sh check`.
The wrapper accepts only `check` or `lint`, resolves the physical checked-in
script before selecting the Makefile, clears all inherited GNU Make control
variables, and invokes fixed system tools. Literal `PYTHON`, `NODE`, and `NPM`
paths remain an explicit caller boundary. Direct GNU Make options, startup
files, `--eval`, and additional `-f` files execute outside this trusted entrypoint.
